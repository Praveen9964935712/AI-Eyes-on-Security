...
export default function LogsTableSimple({ logs, onRefresh }: LogsTableProps) {
  ...
  const getEventIcon = (event: string) => {
    if (event.toLowerCase().includes('unauthorized') || event.toLowerCase().includes('intruder')) {
      return 'ri-user-forbid-line text-red-600';
    } else if (event.toLowerCase().includes('suspicious')) {
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{selectedLog.location}</span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    {selectedLog.confidence && (
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">{selectedLog.confidence}%</span>
                      </div>
                    )}
                    {selectedLog.action && (
                      <div className="flex justify-between">
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {logs.map((log) => (
                  <tr 
                    key={log.id} 
                    className={`hover:bg-gray-50 cursor-pointer ${getEventColor(log.event)}`}
                    onClick={() => setSelectedLog(log)}
                  >
                    <td className="px-3 sm:px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <i className={`${getEventIcon(log.event)} mr-2 text-lg`}></i>
                        <div>
