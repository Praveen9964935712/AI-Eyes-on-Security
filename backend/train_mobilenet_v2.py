...
def main():
    ...
    # Add "Unknown" class with synthetic negative examples
    print("\n3. Adding 'Unknown' class (negative examples)...")
    print("   Generating synthetic unknown faces...")
    
    # Strategy: Use inverted/distorted versions of known faces as unknowns
    unknown_count = 0
    for i in range(min(50, len(all_features) // 3)):  # Add ~16% unknown samples
        # Take a random known face and distort it heavily
        idx = np.random.randint(0, len(all_features))
        original_feature = all_features[idx].copy()
        
        # Add random noise to features (simulate different person)
        noise = np.random.normal(0, 0.5, original_feature.shape)
        unknown_feature = original_feature + noise
        
        all_features.append(unknown_feature)
        all_labels.append("Unknown")
        unknown_count += 1
    
    print(f"   ✅ Added {unknown_count} unknown samples")
